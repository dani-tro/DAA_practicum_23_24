...
int main()
{
    ...
    vector<int64_t> max_suml(n, INT64_MIN);
    vector<int64_t> max_sumr(n, INT64_MIN);
    ...
    for (long long i = 1; i < n; i++)
    {
        for (long long j = 0; j < k; j++)
        {
            ...
            if(max_suml[i-moves[j]] == INT64_MIN)
                continue;
    ...
    for (long long i = n - 2; i >= 0; i--)
    {
        for (long long j = 0; j < k; j++)
        {
            ...
            if(max_sumr[i+moves[j]] == INT64_MIN)
                continue;
    ...
    for (size_t i = 0; i < n; i++)
    {
        ...
        {   
            if(max_suml[i] == INT64_MIN || max_sumr[j] == INT64_MIN)
                continue;
