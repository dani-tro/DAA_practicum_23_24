...
void dfs(vector<long long int> gr[], long long int start, long long int value, long long int curr, int count) {
	...
	for(int i = 0; i < gr[start].size(); ++i){
		if (!visited[gr[start][i]]) {
			...
			dfs(gr, gr[start][i], value, curr, count++);
		}
	}
}
		...
		if (!gr[i].empty()) {
			for (int j = 0; j < gr[i].size(); ++j) {
				for (int k = 0; k < gr[i].size(); ++k) {
					dfs(gr, gr[i][k], gr[i][j], gr[i][k], 0);
					if ((fin == true) && (gr[i][k] != gr[i][j])) {
						return false;
					}
					fin = false;
				}
