...
int main()
{
	...
	priority_queue<tuple<long long, coord, coord>> pq;

	for (int i = 0; i < N; i++)
	{
		for (int j = i + 1; j < N; j++)
		{
			long long distance = (b[i].x - b[j].x) * (b[i].x - b[j].x) + (b[i].y - b[j].y) * (b[i].y - b[j].y);
	...
	while (cnt < N && !pq.empty())
	{
		...
		int f1, f2, f3, f4;
		f1 = find_set(u.x, u.y).x;
		f2 = find_set(v.x, v.y).x;
		f3 = find_set(u.x, u.y).y;
		f4 = find_set(v.x, v.y).y;
