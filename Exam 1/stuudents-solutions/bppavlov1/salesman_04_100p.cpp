...
struct stock {
    ...
    stock(long double kg, long double price, int index) : kg(kg), price(price), index(index) {}
...
struct stockByIndex {
    ...
    stockByIndex(long double kg, long double price, int index) : kg(kg), price(price), index(index) {}
...
int main() {
    ...
    for(; i < n; i++) {
        ...
        if (m == 0) {
            picked[i] = stockByIndex(0, stocks[i].price, stocks[i].index);
            continue;
        }
        if (stocks[i].kg <= m) {
