...
int main(){
    ...
    for(int i = 0; i < n; i++) {
        pairs.push_back({endsFr[i], i+1});
    }

    cout << totalHardness << endl;

    sort(pairs.begin(), pairs.end());

    for(int i = n - 1; i >= 0 && k > 0; i--) {
        totalHardness -= pairs[i].first * min((long long) arr[pairs[i].second - 1], k);
        // cout << "sub " << pairs[i].first * min((long long) arr[pairs[i].second - 1], k) << "  " << 
