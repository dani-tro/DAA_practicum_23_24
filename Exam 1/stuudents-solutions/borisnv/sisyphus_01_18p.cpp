...
bool operator<(const event &x, const event &y) { return x.x < y.x || (x.x == y.x && x.is_end < y.is_end); }
bool operator<(const interval &x, const interval &y) { return x.coef > y.coef; }
